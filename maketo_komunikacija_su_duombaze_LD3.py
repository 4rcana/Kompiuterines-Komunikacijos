...
GPS_Coordinates_Formated = None
GPS_Time_Formated = None
GPS_Satellites = None
...
def write_to_db():
    global Temperature_Value, GPS_Coordinates_Formated, GPS_Time_Formated, GPS_Satellites

    if (GPS_Coordinates_Formated and GPS_Time_Formated
        and (GPS_Satellites >= 0) and (Temperature_Value != 0)):
        conn = connect_db()
        cursor = conn.cursor()
        cursor.execute(f"INSERT INTO {DATABASE_TABLE_NAME} "
                       f"(temperatura, koordinates, laikas, palydovai) "
                       f"VALUES (%s, %s, %s, %s)",
                       (Temperature_Value, GPS_Coordinates_Formated, GPS_Time_Formated, GPS_Satellites))
        conn.commit()
        cursor.close()
        conn.close()

# Funkcija skaityti duomenis iš duomenų bazės ir juos atvaizduoti `From_DB_Data_Display` teksto lauke
# Aktyvuojama paspaudus `Skaityti iš duomenų bazės` mygtuką
def read_from_db():
    global chart_x_data, chart_y_data
    conn = connect_db()
    cursor = conn.cursor()
    ...
    From_DB_Data_Display.config(state=tk.NORMAL)
    From_DB_Data_Display.delete(1.0, tk.END)
    From_DB_Data_Display.insert(tk.END, display_text)
    From_DB_Data_Display.config(state=tk.DISABLED)
    ...
    for i, row in enumerate(rows):
        chart_x_data.append(i)
        chart_y_data.append(row[0])
...
def Display_UART_Data():
    global COM_Port_Selected, UART_Started
    global Temperature_Value, GPS_Coordinates_Formated, GPS_Time_Formated, GPS_Satellites
    if UART_Started and COM_Port_Selected.in_waiting > 0:
        ...
        Temperature_Value = float(data_parts[7])
        ...
        GPS_Coordinates_Formated    = (f"Platuma: {GPS_Coordinate_N_Degrees}° {GPS_Coordinate_N_Minutes:.5f}'\n"
                                       f"Ilguma: {GPS_Coordinate_E_Degrees}° {GPS_Coordinate_E_Minutes:.5f}'"  )
        ...
        # Apdoroti duomenys įrašomi į duomenų bazę.
        write_to_db()
...
def Clear_All():
    ...
    From_DB_Data_Display["state"] = "normal"
    From_DB_Data_Display.delete('1.0', tk.END)
    From_DB_Data_Display["state"] = "disable"
    Chart_Line.set_data([], [])
    Chart_Canvas.draw()
...
Application_window.title('Kompiuterinės Komunikacijos 3 Lab. Darbas')
...
# Duomenų bazės nuskaitymo mygtukas
Read_DB_Button = tk.Button(Application_window, text = "Skaityti iš duomenų bazės", command = lambda: read_from_db())

# COM PORT duomenų atvaizdavimo tekstinis laukelis
COM_Port_UART_Data_Display = tk.Text(Application_window, width=50, height=13, bg="light grey", state='disabled')

# Iš duomenų bazės nuskaitytų duomenų atvaizdavimo tekstinis laukelis
From_DB_Data_Display = tk.Text(Application_window, width=50, height=13, bg="light grey", state='disabled')

# Tekstinių laukelių išvalymo mygtukas
...
Start_Stop_Button.grid(row=3, column=0, columnspan=1, padx=10, pady=10)
Read_DB_Button.grid(row=3, column=1, padx=10, pady=10)
COM_Port_UART_Data_Display.grid(row=4, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
From_DB_Data_Display.grid(row=5, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
...
Chart_Canvas.get_tk_widget().grid(row=4, rowspan=2, column=2, columnspan=3, padx=0, pady=10, sticky="nsew")
